...
import { Bus, Clock, MapPin, CreditCard, Route, Wifi, Globe, Mountain } from 'lucide-react';
    ...
    <div className="flex flex-col md:flex-row h-screen bg-gray-100 font-sans text-gray-800">
      ...
      <div className="md:w-64 bg-slate-900 shadow-2xl relative overflow-hidden transition-all duration-300">
        <div className="p-4 md:p-6 relative z-10 border-b border-slate-700 flex justify-between items-center md:flex-col md:items-start">
          ...
          <button onClick={toggleLanguage} className="md:mt-4 p-2 md:p-1 text-xs rounded-lg text-white bg-slate-800 hover:bg-slate-700 transition-colors flex items-center space-x-1">
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="bg-white shadow-sm p-4 md:p-6 relative flex justify-between items-center border-b border-gray-200">
          ...
          <button onClick={toggleLanguage} className="p-2 text-sm md:hidden rounded-lg text-white bg-red-600 hover:bg-red-700 transition-colors flex items-center space-x-1">
            <Globe className="w-4 h-4" />
            <span>{t.langSwitch}</span>
          </button>
        </div>
        ...
        <div className="p-4 md:p-8">
            ...
            <div className="space-y-6 md:space-y-8">
              ...
              <div className="flex space-x-2 bg-white rounded-full p-1 shadow-inner w-fit">
                <button 
                  ...
                  className={`px-3 md:px-4 py-2 rounded-full transition-all text-sm font-medium ${selectedDirection === 'FromThimphu' ? 'bg-red-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
                >
                  {t.timing.fromThimphu}
                </button>
                <button 
                  onClick={() => setSelectedDirection('ToThimphu')}
                  className={`px-3 md:px-4 py-2 rounded-full transition-all text-sm font-medium ${selectedDirection === 'ToThimphu' ? 'bg-red-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
                >
                  {t.timing.toThimphu}
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-4 md:mb-8">
                {busRoutes.map((route) => (
                  <div key={route.id} className="bg-white rounded-xl p-4 md:p-6 shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                    <div className="flex items-center space-x-2 md:space-x-4 mb-2 md:mb-3">
                      <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${route.color}`}></div>
                      <h3 className="font-bold text-base md:text-lg text-gray-800">{route.name}</h3>
                    </div>
                    <p className="text-xs md:text-sm text-gray-500">{route.destination}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md border-t-4 border-red-600 overflow-hidden">
                ...
                <div className="divide-y divide-gray-200">
                  {busTimings.map((bus, index) => (
                    <div key={index} className="p-4 md:p-6 hover:bg-gray-50 transition-all duration-300">
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
                        <div className="flex items-center space-x-4 md:space-x-6 mb-3 md:mb-0">
                          ...
                          <div>
                            <h4 className="text-base md:text-lg font-bold text-gray-800">{bus.destination}</h4>
                            <p className="text-gray-600 text-xs md:text-sm">{t.timing.bus} #{bus.busNumber}</p>
                            <p className="text-xs md:text-sm text-blue-600">{t.timing.driver}: {bus.driver}</p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-4 md:space-x-6 w-full md:w-auto justify-between md:justify-start">
                          <div className="text-center">
                            <p className="text-xl md:text-2xl font-bold text-red-600">{bus.nextArrival}</p>
                            <p className="text-xs text-gray-500">{t.timing.next}</p>
                          </div>
                          <div className="text-center hidden sm:block">
                            <p className="text-base md:text-lg font-semibold text-gray-600">{bus.following}</p>
                            <p className="text-xs text-gray-500">{t.timing.following}</p>
                          </div>
                          <div className={`px-2 md:px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(bus.status)}`}>
          ...
          {activeTab === 'routes' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {busRoutes.map((route) => (
                <div key={route.id} className="bg-white rounded-xl p-6 md:p-8 shadow-md border-l-4 border-red-600 hover:shadow-lg transition-all duration-300">
                  <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-4">
                    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center ${route.color}`}>
                      <Bus className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    </div>
                    <h3 className="text-lg md:text-xl font-bold text-gray-800">{route.name}</h3>
                  </div>
                  <p className="text-gray-500 text-sm mb-4 md:mb-6">{route.destination}</p>
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="bg-white rounded-xl p-4 md:p-6 shadow-md border-t-4 border-green-600">
                ...
                <div className="space-y-3 md:space-y-4">
                  {liveBuses.map((bus) => (
                    <div key={bus.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-3 md:p-4 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-lg transition-all">
            ...
            <div className="max-w-md mx-auto">
              <div className="bg-white rounded-xl p-6 md:p-8 shadow-md border-t-4 border-red-600">
                <div className="text-center mb-4 md:mb-6">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-gray-800">{t.balance.header}</h3>
                    ...
                    {balanceResult.error ? (
                      ...
                      <div className="text-center">
                        <div className="bg-green-600 text-white p-3 md:p-4 rounded-lg mb-2 md:mb-3">
                          ...
                          <p className="text-2xl md:text-3xl font-bold mt-1">Nu. {balanceResult.balance}</p>
                        </div>
